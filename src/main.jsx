...
import { Recycle, Heart, Brain, Sparkles, Leaf, Accessibility, Menu, X, ArrowRight, Mail, Instagram, Target, Eye, Gem } from 'lucide-react';
import './styles.css';

const sections = ['Início','Sobre','Missão','Impacto','Como funciona','Contato'];

function App(){
  const [active,setActive]=useState('Início');
  const [open,setOpen]=useState(false);
  const go=(id)=>{setActive(id);setOpen(false);document.getElementById(id.toLowerCase().replaceAll(' ','-'))?.scrollIntoView({behavior:'smooth'});};
  return <div className="app">
    <header className="header"><a className="logo" onClick={()=>go('Início')}><span className="logo-mark"><Recycle size={23}/></span><span>Nova<span>Tech</span></span></a>
      <nav>{sections.map(s=><button className={active===s?'active':''} onClick={()=>go(s)} key={s}>{s}</button>)}</nav>
      <button className="menu" onClick={()=>setOpen(!open)}>{open?<X/>:<Menu/>}</button>
    </header>
    {open&&<div className="mobile-nav">{sections.map(s=><button onClick={()=>go(s)} key={s}>{s}</button>)}</div>}
    ...
    <main>
      ...
      </div><div className="hero-art"><div className="orb orb1"/><div className="orb orb2"/><div className="toy-card"><div className="toy-icon"><Brain size={54}/></div><span>brinquedo sensorial</span><strong>feito com propósito</strong></div></div></section>
      ...
      <section id="contato" className="contact"><div><span>05 — FALE COM A NOVATECH</span><h2>Vamos transformar<br/><em>o futuro juntos?</em></h2><p>Quer conhecer o projeto, apoiar nossa iniciativa ou conversar sobre uma parceria?</p></div><div className="contact-links"><a href="mailto:contato@novatech.com.br"><Mail/> contato@novatech.com.br</a><a href="#"><Instagram/> @novatech</a></div></section>
    </main>
    <footer><div className="logo"><span className="logo-mark"><Recycle size={20}/></span><span>Nova<span>Tech</span></span></div><p>Tecnologia descartada. Futuro reinventado.</p><small>© 2026 NovaTech. Economia circular e inclusão pelo brincar.</small></footer>
  </div>
}
