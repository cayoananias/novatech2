...
function App(){
  ...
  return <div className="app">
    <header className="header"><button className="logo" onClick={()=>go('Início')}><span className="logo-mark"><img className="logo-image" src={logo} alt="" /></span><span>NovaTech <span>Toy</span></span></button>
    ...
    {page==='contact'?<ContactPage onBack={()=>go('Início')}/>:<main>
      <section id="início" className="hero"><div className="hero-glow"/><div className="hero-content">
        ...
        <p>A NovaTech Toy transforma resíduos eletrônicos em brinquedos sustentáveis e sensoriais, criando experiências de inclusão, aprendizado e diversão para todas as crianças.</p>
        <div className="actions"><button className="primary" onClick={()=>go('Sobre')}>Conheça a NovaTech Toy <ArrowRight size={18}/></button><button className="secondary" onClick={()=>go('Impacto')}>Nosso impacto</button></div>
      ...
      <section id="sobre" className="section intro"><div className="section-title"><span>01 — SOBRE NÓS</span><h2>Repensar o que<br/>parece <em>descartável.</em></h2></div><div className="intro-copy"><p>A NovaTech Toy nasceu de uma ideia simples: <b>o lixo eletrônico pode ter uma segunda vida.</b> Em vez de terminar em um aterro, componentes e materiais ganham novas formas para estimular a criatividade, a autonomia e a conexão.</p><p>Unimos tecnologia, sustentabilidade e design inclusivo para criar brinquedos que fazem sentido para diferentes formas de brincar.</p></div></section>
      ...
      <section id="contexto" className="section context"><div className="section-title centered"><span>03.5 — CONTEXTO</span><h2>Por que esse problema<br/><em>importa tanto?</em></h2></div><div className="context-grid"><ContextCard label="Cenário global" value="62 milhões de toneladas" text="Volume total de resíduos gerados no mundo. Cerca de 48 milhões de toneladas não entram nas rotas formais de reciclagem e acabam no lixo comum ou em aterros."/><ContextCard label="Valor perdido no mundo" value="US$ 62 bilhões" text="Esse montante representa metais e materiais nobres que poderiam ser recuperados, mas seguem sem aproveitamento."/><ContextCard label="Cenário no Brasil" value="2,4 milhões de toneladas/ano" text="O país está entre os maiores produtores do mundo, mas cerca de 97% desse total não recebe o destino correto."/><ContextCard label="Perda no Brasil" value="Centenas de milhões de dólares" text="A falta de reciclagem em larga escala e de pontos de coleta suficientes faz o país desperdiçar matérias-primas preciosas todos os anos."/></div></section>
      ...
      <section className="contact"><div><span>05 — FALE COM A NOVATECH TOY</span><h2>Vamos transformar<br/><em>o futuro juntos?</em></h2><p>Quer conhecer o projeto, apoiar nossa iniciativa ou conversar sobre uma parceria?</p></div><button className="contact-cta" onClick={contact}>Falar com a equipe <ArrowRight size={18}/></button></section>
    </main>}
    <footer><img className="footer-logo" src={logo} alt="Logo NovaTech Toy"/><div className="footer-copy"><p>Tecnologia descartada. Futuro reinventado.</p><small>Projeto realizado por alunos da E.E. João Ramalho.</small><small>© 2026 NovaTech Toy. Economia circular e inclusão pelo brincar.</small></div></footer>
  </div>
}
...
function ContextCard({label,value,text}){return <article className="context-card"><span>{label}</span><strong>{value}</strong><p>{text}</p></article>}
